 ...
 */
        void ScoreConverter::addNote(const Note &note) {
            if (note.isRest()) { // rest
                ...
                if (!score.addNote(note.getDuration() * BASE_DIVISIONS / divisions, note.getLyric(),
                                   pitch.getOctave() * 12 + pitch.getStep(), note.hasAccent(), note.hasStaccato(),
                                   tieType, slurType, syllabicType, note.hasBreathMark())) {
 ...
 */
    bool
    Sinsy::addNote(size_t duration, const std::string &lyric, size_t pitch, bool accent, bool staccato, TieType tieType,
                   SlurType slurType, SyllabicType syllabicType, bool breath) {
        if (duration <= 0) {
