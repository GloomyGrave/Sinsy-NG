...
void usage() {
    std::cout << "The Formant-Based Singing Voice Syntheis System \"Sinsy\"\n"
              << "Version 0.0.1 (http://Github.GloomyGhost.com/Sinsy-NG/)\n"
              << "\tUSAGE:\n"
...
public:
    ...
    virtual bool
    addNote(size_t duration, const std::string &lyric, size_t pitch, bool accent, bool staccato, sinsy::TieType tieType,
            sinsy::SlurType slurType, sinsy::SyllabicType syllabicType, bool breath = false) {
...
class CommandHandler {
    ...
    void note() {
        ...
        if (argv.size() > 8)
            ecantorix.addNote(atoi(argv[1].c_str()), argv[2], atoi(argv[3].c_str()),
                              (bool) atoi(argv[4].c_str()), (bool) atoi(argv[5].c_str()),
                              (sinsy::TieType) atoi(argv[6].c_str()), (sinsy::SlurType) atoi(argv[7].c_str()),
                              (sinsy::SyllabicType) atoi(argv[8].c_str()), breath);
    }
